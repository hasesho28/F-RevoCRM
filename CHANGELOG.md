...
## セキュリティ関連
* #1445 複数のセキュリティ関連修正
* #1421 複数の脆弱性対応の実施（bootstrap v3.4.1更新、moment.js更新、.htaccessセキュリティ強化、Serverバージョン非公開化、HTTPヘッダーセキュリティ強化）

## 新機能
* #1447 更新履歴に削除と復元の情報を表示する機能を追加
* #1440 WebComponents版クイック作成の実装
* #1436 イベント（カレンダー）に「共有メモ」項目を追加
* #1434 カレンダーのマイグループと役割の表示非表示を記憶する機能
* #1419 他人の作成した活動を変更する際に確認メッセージを表示
* #1418 編集ページに編集不可項目を表示する機能を実装
* #1398 多要素認証の記憶機能を追加
* #1378 ログイン履歴にpassword, totp, passkeyのどれで認証したかを記録
* #1373 ログイン失敗でロックにかかる仕組みをMFA以外でも有効化
* #1362 ダッシュボードのウィジェットにIFrameを追加
* #1359 参加者なら他者の活動も削除できる対応
* #1332 システム管理者が他ユーザーのカレンダー設定を変更できるよう対応

## 機能改善
* #1426 マイグレーションシステムの改修 - 基底クラスの最適化、キャッシュ問題対応、SQLエラーハンドリング改善
* #1427 ユーザーインポート時の「役割」項目を翻訳後名称で当てるよう修正
* #1408 レポート出力画面でHeaderを固定するように修正
* #1406 Importにて存在しないPicklistがImportされた場合、エラーで保存しないよう修正
* #1366 レポート出力時のExcelの拡張子がxlsからxlsx形式で保存されるよう修正
* #1346 マイグレーション機能の強化 - 単体実行時の-dオプション追加
* #1329 マイグレーションスクリプトの登録方法変更
* #1303 複数の関連項目をインポートできるように修正
* #516 活動招待メールのiCalendar TIMEZONEをAsia/Tokyoに修正

## 不具合修正
* #1455 活動の編集画面において、繰り返しか否かの判定がYes/Noで判定されているが、翻訳がかかった値と比較が行われてしまっている問題の修正
* #1454 活動のクイック作成から詳細入力画面に遷移した際のURLと一覧画面から活動の詳細入力を開いた際のURLが異なる問題の修正
* #1451 インストール画面Step3にてボタン操作が効かない不具合の修正
* #1446 カレンダーの一括削除などを行うとエラーが発生する不具合の修正
* #1444 タグ設定の修正
* #1443 初期設定されている一部のワークフローが編集時にエラーとなっている問題の対応
* #1442 スマートフォンポップオーバー画面表示順の仕様変更、編集画面遷移の端末対応
* #1433 グループメンバースケジュールの権限アクセス制御の修正
* #1428 bootstrapをv3.4.1に更新したことにより、popoverが正常に動作できなくなっていた問題を修正
* #1420 スマホでカレンダー表示時に日表示になるように修正
* #1417 カレンダーで終日でない活動をD&Dで終日領域に移動させた際の権限エラー修正
* #1415 受注モジュールの繰り返し請求関連の修正
* #1414 スマホサイズ画面でユーザー詳細画面でアイコンが隠れる不具合の修正
* #1412 画面上からご担当者様（連絡先）が保存できない不具合の修正
* #1411 タグの動作が統一されていない不具合の修正
* #1410 画面左上のモジュール表示がずれている問題の修正
* #1407 「項目の値の更新」のチェックボックスの表示の修正
* #1405 カレンダー複製時に終了時間が変更されないよう修正
* #1404 テンプレート内で利用するPHP関数を事前に登録するよう修正
* #1403 項目編集一覧画面で横2幅取る項目の表示を一致するよう修正
* #1402 関連先レコードが削除されている場合のメッセージ表示
* #1396 モジュール権限無効をアクション権限有効より優先する修正
* #1395 チケットの関連リストに資産・レンタル管理が重複表示される問題を修正
* #1394 管理者(ID=1)を削除不可対応
* #1393 ユーザー永久削除時に関連するMFA情報も全て削除するよう対応
* #1391 リストで関連項目検索ボタンの機能修正
* #1386 vtiger_tabのname文字数を増加
* #1382 メール送信モーダルのモジュール検索ボタンが機能しない問題の修正
* #1377 ログイン画面エラーメッセージの修正
* #1370 共有カレンダーで他者の活動削除時、権限エラー時に「成功」と表示される問題の修正
* #1368 カレンダー一覧でmodnavが消える不具合の修正
* #1364 ユーザー名の変更時にユーザー登録時にない文字制限がある問題の修正
* #1363 カスタムビューのバリデーション修正
* #1360 パーセント項目の表示修正
* #1357 カレンダー一覧レイアウト修正
* #1356 リスト作成時の抽出条件の選択肢整理
* #1348 FPLoginレイアウト乱れ修正
* #1344 ユーザー削除時、一度キャンセルすると再表示できないバグの修正
* #1338 カレンダー一覧画面でEnterキーによる検索が動作しない問題の修正
* #1336 WebフォームからPOSTを受け取るとFatal Errorが発生する問題の修正
* #1334 Calendar/Activity.phpのnull値チェックを追加し、エラーハンドリング改善
* #1328 datePickerが見切れる不具合を修正
* #1325 現在のページをエクスポートした際にダウンロードが完了しない不具合を修正
* #1322 クイック入力の日報アイコンの乱れを修正
* #1307 BOM付きUTF-8で最初の項目の自動選択がされない不具合の修正
* #1296 Emailに複数ファイルを添付できない不具合の修正
* #1242 クイック作成メニューのレイアウトが崩れる不具合の修正
* #1232 全検索でリードを出すとき、Convertedされたレコードが正しく紐づかない問題の修正
* #814 関連項目として価格表を設定できない不具合を修正
* #812 PDFテンプレートのリスト画面でポップオーバーが消えない不具合の修正
* #782 日報モジュールにおいてインポートができない問題を修正
* #777 活動の参加者登録の際にSQLエラーが発生しないように修正
* #619 項目設定で必須の付け外しをした際の*の付与位置ずれ修正
* #608 カレンダー種別でプロジェクト表示時に英語表記になる問題の修正

## 翻訳・文言修正
* #1380 接頭子→接頭辞に翻訳変更
* #1327 「連絡除外」の項目名を「連絡不可」に変更
* #1316 二要素認証に関するシステム変数の備考を修正
* #1314 二要素認証周りの文言を調整
* #1122 受注モジュールの周期項目の選択肢に「半期」を初期追加
* #1078 活動周期の文言修正
* #788 ワークフローのアクションを追加する際の翻訳漏れを修正
* #785 ブックマークの一覧画面の翻訳漏れの修正
* #761 メニュー設定の翻訳漏れの修正
* #760 Rssの翻訳漏れの修正

## 開発環境・その他
* #1413 .claude設定を.gitignoreに追加
* #1341 Skype項目の新規追加を不可にする対応
* #1318 PHPのDockerfileがBuildできない不具合の修正
* docker-compose.yamlからcompose.yaml(v2準拠)に名称変更
* imap関係のライブラリがインストールできない不具合の修正
